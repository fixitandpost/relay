...
import type { RTSPNodeData } from '../types'

export function RTSPOutputNode({ id, data }: NodeProps) {
  const { updateNodeData } = useFlowStore()
  const d = data as RTSPNodeData
  const url = `rtsp://localhost:${d.port}/${d.path}`
      ...
      <div className="node-body">
        ...
        <div className="node-field">
          ...
          <input
            type="number"
            value={d.port}
            onChange={(e) => updateNodeData(id, { port: parseInt(e.target.value) || 8554 })}
          />
        </div>
        <div className="node-field">
